...
def kalman_filter(A,B,H,Z,u,R,Q):
    ...
    # initial estimative
    # x_init = Z[0]
    x_init = Z[0] #+ [1,10e-3,1,10e-3,1,10e-3]
    ...
    y = np.zeros((nt,nx))             # filter innovation
    ...
    for i in range(1,nt):
        ...
        # prediction stage
        x_pred[i] = np.matmul(A,x_est[i-1]) + np.matmul(B,u[i])
        P_pred[i] = np.matmul(np.matmul(A,P_est[i-1]),A.T) + Q

        # estimation stage
        # [MODIFICATION] only run when a new measurement arrives
        if not np.array_equal(Z[i],Z[i-1]):
            y[i] = Z[i] - np.matmul(H,x_pred[i])

            # Kalman gain
            S = np.matmul(np.matmul(H,P_pred[i]),H.T)+R
            K[i] = np.matmul(np.matmul(P_pred[i],H.T), inv(S))

            # prediction
            x_est[i] = x_pred[i] + np.matmul(K[i],y[i])
            P_est[i] = np.matmul((I - np.matmul(K[i],H)),P_pred[i])
            # P_est[i] = np.matmul(np.matmul(I-K[i],P_pred[i]),(I-K[i]).T) + np.matmul(np.matmul(K[i],R),K[i].T)
        else:
            x_est[i] = x_pred[i]
            P_est[i] = P_pred[i]

    return x_est, P_est, y