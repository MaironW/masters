...
def particle_filter(A, B, Z, u, R, Q, n_particles):
    ...
    dx_range = [Z[0, 0]-R[0,0], Z[0, 0]+R[0,0]]
    vx_range = [Z[0, 1]-R[1,1], Z[0, 1]+R[1,1]]
    dy_range = [Z[0, 2]-R[2,2], Z[0, 2]+R[2,2]]
    vy_range = [Z[0, 3]-R[3,3], Z[0, 3]+R[3,3]]
    dx_init  = np.random.uniform(dx_range[0], dx_range[1], size=n_particles)
    vx_init  = np.random.uniform(vx_range[0], vx_range[1], size=n_particles)
    dy_init  = np.random.uniform(dy_range[0], dy_range[1], size=n_particles)
    vy_init  = np.random.uniform(vy_range[0], vy_range[1], size=n_particles)
    states   = np.vstack((dx_init,vx_init,dy_init,vy_init))
    ...
    for i in range(n_iters):
        ...
        new_states = np.matmul(A, states) + np.matmul(B,u[i]).reshape(n_states,1) + process_random_variables
